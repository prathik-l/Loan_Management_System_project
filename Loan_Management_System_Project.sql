...
case 
when applicantincome > 15000 then "Grade A Customer"
...
end as Grade from customer_income;
...
select * from monthly_interest_percentage_table;
...
create table loan_status (loan_id varchar(20),
customer_id varchar(20),loan_amount text,
loan_amount_term int,cibil_score int);
alter table loan_status modify loan_id varchar(20) primary key;
alter table loan_status modify loan_amount varchar(25);
...
delimiter ;
drop trigger loan_amt_null;

-- trigger statement level
create table cibil_score_status_details(
loan_id varchar(20) primary key,
loan_amount int,
cibil_score int,
cibil_score_status varchar(50));

alter table cibil_score_status_details modify loan_amount varchar(25); 
...
insert into loan_status (loan_id,customer_id,loan_amount,loan_amount_term,cibil_score) values
...
select * from cibil_score_status_details;
...
-- 3 table joins (customer_det,country_state,region_info)
...
select *,row_number() over() from customer_info;

-- joining 2 tables (customer_interest_analysis, loan_status)
...
select *,row_number() over() from customer_loan_info;
...
-- select i.customer_id,i.Loan_Id,i.customer_name,i.Gender,i.Age,i.Married,i.Education,i.Self_Employed,i.Region_id,
-- i.Postal_Code,i.Segment,i.State,i.region,
-- r.applicantincome, r.coapplicantincome,r.property_area,r.loan_status,r.grade,r.monthly_interest_percentage,  
-- r.loan_amount,r.cibil_score,r.cibil_score_status,r.monthly_interest_amount,r.annual_interest_amount,r.loan_amount_term
-- from result_table r right join customer_info i on i.Loan_Id = r.loan_id;
-- drop table mismatch_table;

-- select * from mismatch_table;

-- select * from mismatch_table where loan_amount is null;

-- mismatch table

create table mismatch_table1
select c.Customer_id,c.Load_Id,c.Customer_name,c.Region_id,c.Postal_Code,c.Segment,c.State,
r.region from country_state c right join region_info r on c.Region_id = r.Region_id;

select * from mismatch_table1;

-- percentage change
alter table result_table modify monthly_interest_percentage varchar(10);
update result_table set monthly_interest_percentage = concat(monthly_interest_percentage,'%');
